...
export function WhatsAppButton() {
  ...
  return (
    <a
      ...
      className="fixed bottom-6 left-6 z-50 flex items-center gap-2.5 group flex-row-reverse"
    >
      {/* Tooltip */}
      <span
        className={`hidden sm:block bg-white text-gray-800 text-sm font-semibold px-3 py-2 rounded-xl shadow-md border border-gray-100 whitespace-nowrap transition-all duration-200 ${
      ...
      {/* Button */}
      <span className="relative flex items-center justify-center w-12 h-12 rounded-full bg-[#25D366] shadow-md hover:shadow-lg transition-all duration-200 group-hover:scale-105 group-active:scale-95">
        {/* Pulsing ring */}
        <span className="absolute inset-0 rounded-full bg-[#25D366] opacity-40 animate-ping" style={{ animationDuration: '3s' }} />
        {/* WhatsApp glyph */}
