      ...
      <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
        ...
        <div style="background: #FAFAF9; padding: 24px; border-radius: 0 0 12px 12px; border: 1px solid #E5E7EB; border-top: none;">
          ...
          <div style="margin-top: 24px; padding-top: 16px; border-top: 1px solid #E5E7EB;">
            <a href="${process.env.NEXT_PUBLIC_SITE_URL}/admin/leads" style="background: #0D9488; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-size: 14px; font-weight: 600;">View in Admin →</a>
          </div>
        </div>
      </div>
    `,
...
export async function sendLeadWelcome(data: { name: string; email: string }) {
  ...
  await getResend().emails.send({
    ...
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
        <div style="background: linear-gradient(135deg, #0D9488, #0F766E); padding: 32px; border-radius: 12px 12px 0 0; text-align: center;">
        ...
        <div style="background: white; padding: 32px; border-radius: 0 0 12px 12px; border: 1px solid #E5E7EB; border-top: none;">
          ...
          <a href="${process.env.NEXT_PUBLIC_CALENDLY_URL}" style="background: #0D9488; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-size: 14px; font-weight: 600; display: inline-block;">Book a Free Call →</a>
